...
export default function RegisterRecep({ isLog }) {
    ...
    async function handleClickSubmit() {
        ...
        if (name === '' || msgName !== '') refs.rfName.current.focus();
            ...
            try {
                const apiurl = process.env.REACT_APP_API_URL + '/register-recep';
                ...
                if (response.status !== 200) {
                    if (response.status === 204) alert("El correo|usuario ya están asociados a otra cuenta");
    ...
    return (
        <div className={styles.formContainer}>
            ...
            <div className={styles.formSubmit}>
                {isLog && (
                    <button className={styles.buttonSubmit} onClick={handleClickSubmit}>Registrar Recepcionista</button>
                )}
            </div>
        </div>
    );
